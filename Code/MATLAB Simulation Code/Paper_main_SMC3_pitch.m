...
%     end
    ...
    s1 = beta11 * sum11 + beta12*e1;%滑模面
    
    dUa1 = (Ua(2)-lUa1)/h; %期望运动控制器的一阶导数
    lUa1 = Ua(2);
    
    D1 = s1*(beta11*8.33*(1+e1^2)*atan(e1)-8.33*beta12*dUa1);
    D2 = s1*(beta12*(Thetafq*basfq'));
    dlambda1 = -0.01*lambda1+(K12*beta12*s1-D2-D1)/(sqrt(1+lambda1*lambda1));
...
%     M = M+(h/6).*(dt1+2.*dt2+2.*dt3+dt4);
    ...
    [d1,] = FLS(Thetafq,state,s1,1,beta12);
    [d2,] = FLS(Thetafq+0.5*h.*d1,state,s1,1,beta12);
    [d3,] = FLS(Thetafq+0.5*h.*d2,state,s1,1,beta12);
    [d4,basfq] = FLS(Thetafq+h.*d3,state,s1,1,beta12);
    ...
    s2 = beta21 * sum21 + beta22 * sum22;
    
    dUa2 = (Ua(3)-lUa2)/h; %期望运动控制器的一阶导数
    lUa2 = Ua(3);
    
    D1 = s2*(8.33*beta21*(1+e2^2)*atan(e2)-8.33*beta22*dUa2);
    D2 = s2*(beta22*(Thetafr*basfr'));
    dlambda2 = -0.01*lambda2+(K22*beta22*s2-D2-D1)/(sqrt(1+lambda2*lambda2));
...
%     N = N+(h/6).*(dt1+2.*dt2+2.*dt3+dt4);
    ...
    [d1,] = FLS(Thetafr,state,s2,2,beta22);
    [d2,] = FLS(Thetafr+0.5*h.*d1,state,s2,2,beta22);
    [d3,] = FLS(Thetafr+0.5*h.*d2,state,s2,2,beta22);
    [d4,basfr] = FLS(Thetafr+h.*d3,state,s2,2,beta22);
